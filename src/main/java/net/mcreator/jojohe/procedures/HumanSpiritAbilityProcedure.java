...
public class HumanSpiritAbilityProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("UltimateCooldown") == 0) {
			...
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal("Human Spirit!"), true);
			if (world.isClientSide()) {
				if (entity instanceof AbstractClientPlayer player) {
					...
					if (animation != null && !animation.isActive()) {
						animation.setAnimation(new KeyframeAnimationPlayer(PlayerAnimationRegistry.getAnimation(new ResourceLocation("jojohe", "powerup"))));
					}
				}
			}
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("jojohe:humanspirit")), SoundSource.NEUTRAL, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("jojohe:humanspirit")), SoundSource.NEUTRAL, 1, 1, false);
				}
			}
		} else {
			if (entity.getPersistentData().getDouble("UltimateCooldown") > 0) {
